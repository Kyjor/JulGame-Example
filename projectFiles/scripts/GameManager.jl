...
function Base.getproperty(this::GameManager, s::Symbol)
    ...
    elseif s == :get
        function ()
            ...
            try
                @async begin
                        this.roomState = Firebase.realdb_getRealTime("/players", this.user["idToken"])
                end
                    sleep(0.001)
            catch e
                print(e)
    ...
    elseif s == :updatePos
        function (position)
            ...
            @async Firebase.realdb_putRealTime("/players/$(this.user["localId"])/$(this.playerId)", this.localPlayerState, this.user["idToken"])
        end
