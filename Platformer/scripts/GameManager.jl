...
function Base.getproperty(this::GameManager, s::Symbol)
    if s == :initialize
        function()
            ...
            push!(MAIN.scene.textBoxes, TextBoxModule.TextBox("test", joinpath(pwd(), ".."), joinpath("VT323", "Vt323-Regular.ttf"), 256, Math.Vector2(), Math.Vector2(), Math.Vector2(), "test", false))
        end
    elseif s == :update
        function(deltaTime)
        end
